...
export function ProductParallaxCard({ product }: ProductParallaxCardProps) {
  ...
  return (
    <Link href={href} className="group block">
      ...
      <div className="relative w-full aspect-square bg-white border border-[#E0E0E0] group-hover:border-[#1B6240] group-hover:shadow-sm transition-all duration-300 overflow-hidden">
        {product.image && (
          <div className={`absolute inset-0 flex items-center justify-center ${product.brand === "motortronics" ? "p-10" : product.brand === "athena-valve" ? "p-8" : "p-6"}`}>
            <div className="relative w-full h-full">
              <Image
                src={product.image}
                alt={product.name}
                fill
                className={`${product.brand === "motortronics" || product.category === "condition-monitor" ? "object-contain" : "object-contain"} object-center`}

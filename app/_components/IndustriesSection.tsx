...
const _allProducts = getAllProducts();

const INDUSTRIES = INDUSTRIES_RAW.map((ind) => {
  const keywords = INDUSTRY_KEYWORDS[ind.title] ?? [];
  const matching = _allProducts.filter((p) =>
    p.applications?.some((app) =>
      keywords.some((kw) => app.toLowerCase().includes(kw.toLowerCase()))
    )
  );
  return {
    ...ind,
    ...
    <section className="w-full bg-background py-8 md:py-16 lg:py-24">
      <div className="w-full px-4 md:px-8 lg:px-12 xl:px-20 mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          {/* Left Column: Section heading + vertical tab list */}
